...
Define the parameter µx as the average of the control population. We estimate thisparameter with the 
...
--> **Correct**: The CLT tells us that the distribution of Xbar is approximately normal with mean **µX or µx** and standard deviation σx/√n, where σx is the population standard deviation and n is the sample size (12 in this case).  
...
--> **Answer:** σx is estimated as √n * standard deviation of the sample, based on CLT.
...
difference between Xbar and µX is greater than 5.21 ounces. We can standardize this difference using
the estimated standard deviation from CLT, which is σx/√n. The standardized variable would be 
`Z = (Xbar - µX) / (σx/√n)`. We want to find `P(|Z| > 5.21 / (σx/√n))`. This can be calculated 
using the cumulative distribution function of the normal distribution.

```{r}
z_score <- 5.21 / (popsd(X) / sqrt(length(X)))
pnorm(q = -z_score, mean = 0, sd = 1) + (1 - pnorm(q = z_score, mean = 0, sd = 1))
```
...
σY. This implies that the difference `Ybar−Xbar` has mean 0 (there is no difference). We described that the standard deviation of 
this statistic (the standard error) is SE(Xbar−Ybar) = sqrt(σx^2/n + σy^2/n) and that
...
Under the null hypothesis that there is no difference between the population averages, the difference between
the sample averages `Ybar−Xbar`, with `Ybar` and `Xbar` being the sample average for the two diets respectively,
is approximated by a normal distribution centered at 0 (there is no difference) and with standard error SE(Xbar−Ybar) 
= sqrt(σx^2/n + σy^2/n).

```{r}
σx <- popsd(X) * sqrt(length(X))
σy <- popsd(Y) * sqrt(length(Y))
SE <- sqrt(σx^2/length(X) + σy^2/length(Y))
SE
```
